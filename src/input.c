...
static void on_cursor_motion_absolute(struct wl_listener* listener, void* data)
{
    ...
    struct wlr_scene_node* node =
        wlr_scene_node_at(&comp->root_scene->tree.node, ev->x, ev->y, &x, &y);

    // wlr_cursor_set_xcursor(comp->cursor, comp->cursor_manager, "default");
    // wlr_cursor_move(comp->cursor, &ev->pointer->base, 0.1, 0.1);

    if (node && node->type == WLR_SCENE_NODE_BUFFER)
    {
        struct wlr_scene_surface* surf =
            wlr_scene_surface_try_from_buffer(wlr_scene_buffer_from_node(node));

        struct wlr_keyboard* keyboard =
            &comp->keyboard_group.wlr_keyboard_group->keyboard;

        if (surf->surface != comp->seat->pointer_state.focused_surface)
        {
            wlr_seat_pointer_notify_enter(comp->seat, surf->surface, 0, 0);
    ...
    wh_log(DEBUG, "absolute: %f %f", x, y);
    wlr_seat_pointer_notify_motion(comp->seat, now_ms, x, y);

    /* 0 for virtual pointer */
    // if (!ev->time_msec)
    // {
    //     wlr_cursor_warp_absolute(
    //         comp->cursor, &ev->pointer->base, ev->x, ev->y
    //     );
    // }
...
static void on_cursor_button(struct wl_listener* listener, void* data)
{
    ...
    wh_log(
        DEBUG,
        "cursor: button %f %f",
        comp->seat->pointer_state.sx,
        comp->seat->pointer_state.sy
    );
...
static void on_cursor_axis(struct wl_listener*, void*)
{
    wh_log(DEBUG, "cursor: axis");
}
...
static void on_keyboard_key(struct wl_listener* listener, void* data)
{
    ...
    wlr_seat_keyboard_notify_key(
        comp->seat, ev->time_msec, ev->state, ev->state
    );
}
...
static int wh_input_keyboard_group_init(WhaleCompositor* comp)
{
    ...
    wlr_keyboard_set_repeat_info(
        &comp->keyboard_group.wlr_keyboard_group->keyboard, 25, 600
    );
    ...
    // LISTEN(
    //     &group->wlr_group->keyboard.events.modifiers,
    //     &group->modifiers,
    //     keypressmod
    // );
